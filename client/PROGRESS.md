...
- [ ] C1.3 Solution view, prepared refutation navigation, and retry that retains errors. Partial: the interpreter and synthetic fixture support false branches and refutation navigation, but the published Cho pack contains zero wrong edges, zero failure terminals, and zero prepared refutations. Full solution view and practice retry also remain. The visible hint button was removed following product feedback.
...
- [ ] C3.2 Daily queue: due → limited new; hinted/wrong/solution = Again. Partial: a persisted collection run shuffles once, and the Client can record correct/wrong, repeat mistakes, and reset local state. The current Cho pack cannot reach the wrong path because it contains no wrong branches, so its one-attempt behavior and mistake statistics do not meet the product requirement. A separate `/statistics` page shows correct/wrong/unseen/accuracy. The SRS daily policy is not connected yet.
- [ ] C3.3 Export and reproducible replay projection.
- [ ] C3.4 First authorized and verified catalog of 100–300 problems. **Partial:** a local restricted Cho candidate catalog contains 861 problems; it is neither an authorized public catalog nor expert verified, and the 2026-09-18 audit found that it is unsuitable for graded one-attempt practice.
