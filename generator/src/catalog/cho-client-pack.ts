...
export async function buildChoClientProblem(input: {
  ...
  const nodes = buildLineNodes(input.position, input.audit.sourceLine, input.audit.expectedRootMoves, selected.goalKind);
  const problem: ProblemV1 = {
    ...
    viewport: viewport(input.position, [
    ...
    tags: ['life-and-death', 'cho-elementary', 'restricted-local', 'candidate', input.corpus, input.audit.status],
  };
...
function viewport(position: Position, linePoints: number[]): ProblemV1['viewport'] {
