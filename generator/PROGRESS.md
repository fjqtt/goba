...
- [ ] G3.1 Branch expansion, alternatives, wrong branches/refutations, and measured coverage. **Partial:** all 845 exact-linked Cho positions passed the GNU Go batch audit. It saved 3,327 trees; the full source line exists for 374/723 records, but only 305 selected trees are fully legal. Every branch is replayed and extra/missing moves remain `unclassified`. KataGo checked 2,642 states at every available key move with 0 API errors. A terminal verifier, normalized ProblemV1, and refutation expansion are still missing.
